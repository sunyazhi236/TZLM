...
@interface TransferDetailViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>{
...
static NSString *const TransCell = @"TransferDetailCell";
static NSString *const FDUCCell = @"FinancialDetailUserCommentCell";
- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.title = @"奥迪A4L  2013款";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    TransferDetailCell *transCell=[tableView dequeueReusableCellWithIdentifier:TransCell];
    FinancialDetailUserCommentCell *fducCell=[tableView dequeueReusableCellWithIdentifier:FDUCCell];
    if (indexPath.section == 0) {
        if (transCell == nil) {
            transCell=[[[NSBundle mainBundle]loadNibNamed:TransCell owner:self options:nil]lastObject];
        }
        transCell.selectionStyle = UITableViewCellSelectionStyleNone;
        return transCell;
    }
        ...
        fducCell.selectionStyle = UITableViewCellSelectionStyleNone;
        return fducCell;
    }
...
//    TransferDetailCell *transCell=[tableView dequeueReusableCellWithIdentifier:TransCell];
//        TransferDetailCell *transCell =(TransferDetailCell *)([_tableView dequeueReusableCellWithIdentifier:TransCell]);
 UITableViewCell *transCell = [self tableView:_tableView cellForRowAtIndexPath:indexPath];
    transCell.backgroundColor = [UIColor greenColor];
    if (indexPath.section == 0) {
        return transCell.frame.size.height;
    }else{
        return kAutoHEX(80);
    }
